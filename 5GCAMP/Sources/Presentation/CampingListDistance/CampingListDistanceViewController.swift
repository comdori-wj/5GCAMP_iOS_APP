...
final class CampingListDistanceViewController: UIViewController, WebViewConfigurable {
    @IBOutlet weak var webView: WKWebView!
    var locationManager: CLLocationManager!
    private let campingListDistanceViewModel = CampingListDistanceViewModel()
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        campingListDistanceViewModel.loadWebView()
        print("거리순 캠핑장 뷰 진입")
    }
    ...
    private func setUpBindings() {
        campingListDistanceViewModel.webViewRequest.bind { [weak self] request in
            if let request = request {
                self?.webView.load(request)
            }
        }
        
        campingListDistanceViewModel.errorMessage.bind { [weak self] message in
...
extension CampingListDistanceViewController: WKUIDelegate {
...
extension CampingListDistanceViewController: WKNavigationDelegate {
