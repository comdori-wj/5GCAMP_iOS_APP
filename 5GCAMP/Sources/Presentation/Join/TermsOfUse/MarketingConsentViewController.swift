...
// MARK: - 이벤트 / 마케팅 수신동의 약관 -
final class MarketingConsentViewController: UIViewController {
    @IBOutlet weak var marketingConsentWebView: WKWebView!
    ...
    private func loadTerms() {
        ...
        session.dataTask(with: requset) { [weak self] data, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    return print("[이벤트 / 마케팅 수신동의] 네트워크 오류:", error.localizedDescription)
                }
                
                guard let httpResponse = response as? HTTPURLResponse else {
                    return print("[이벤트 / 마케팅 수신동의] 잘못된 응답 형식")
                }
                
                print("[이벤트 / 마케팅 수신동의] HTTP Status Code: \(httpResponse.statusCode)")
                ...
                if let extractedContent = self?.extractAgreementContent(from: htmlString) {
                    ...
                    self?.marketingConsentWebView.loadHTMLString(formattedHTML, baseURL: nil)
                } else {
    ...
    private func extractAgreementContent(from htmlString: String) -> String? {
        ...
        for pattern in patterns {
            do {
                ...
                print("[이벤트 / 마케팅 수신동의]정규식 오류: \(error.localizedDescription)")
            }
        }
        
        if let divContent = extractDivContent(from: htmlString, divId: "ag6") {
            return divContent
        }
