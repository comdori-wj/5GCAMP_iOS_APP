...
extension PrivacyPolicyViewController {
    private func loadTerms() {
        ...
        session.dataTask(with: requset) { [weak self] data, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    return print("[개인정보 수집 및 이용동의 약관] 네트워크 오류:", error.localizedDescription)
                }
                
                guard let httpResponse = response as? HTTPURLResponse else {
                    return print("[개인정보 수집 및 이용동의 약관] 잘못된 응답 형식")
                }
                
                print("[개인정보 수집 및 이용동의 약관] HTTP Status Code: \(httpResponse.statusCode)")
                ...
                if let extractedContent = self?.extractAgreementContent(from: htmlString) {
                    ...
                    self?.privacyPolicyWebView.loadHTMLString(formattedHTML, baseURL: nil)
                } else {
    ...
    private func extractAgreementContent(from htmlString: String) -> String? {
        // 개선된 정규식 패턴 - 백틱과 작은따옴표 모두 지원
        let patterns = [
            "var agreetext = \\{[^{]*?3: `([\\s\\S]*?)`[^}]*?\\};",  // 백틱 버전
            "var agreetext = \\{[^{]*?3: '([\\s\\S]*?)'[^}]*?\\};",  // 작은따옴표 버전
            "var agreetext = \\{[^{]*?3: \"([\\s\\S]*?)\"[^}]*?\\};" // 큰따옴표 버전
        ]
            ...
            do {
                ...
                if let match = regex.firstMatch(in: htmlString, options: [], range: range) {
                    ...
                    if matchRange.location != NSNotFound {
                        // 추출된 HTML 내용에서 이스케이프된 문자 처리
                        let extractedHTML = nsString.substring(with: matchRange)
                        return unescapeHTML(extractedHTML)
                    }
                }
            } catch {
                print("[개인정보 수집 및 이용동의 약관]정규식 오류: \(error.localizedDescription)")
            }
        }
        
        // 정규식으로 찾지 못한 경우 div 태그로 직접 찾기
        if let divContent = extractDivContent(from: htmlString, divId: "ag3") {
            return divContent
        }
