//
//  OptionalPrivacyPolicyViewController.swift
//  5GCAMP
//
...
// MARK: - 개인위치기반서비스 이용동의(필수 사항) 약관 -
final class LocationBasedServiceConsentViewController: UIViewController {
    @IBOutlet weak var locationBasedServiceConsentWebView: WKWebView!
...
extension LocationBasedServiceConsentViewController {
    private func loadTerms() {
        ...
        session.dataTask(with: requset) { [weak self] data, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    return print("[개인위치기반서비스 이용동의(필수 사항)] 네트워크 오류:", error.localizedDescription)
                }
                
                guard let httpResponse = response as? HTTPURLResponse else {
                    return print("[개인위치기반서비스 이용동의(필수 사항)] 잘못된 응답 형식")
                }
                
                print("[개인위치기반서비스 이용동의(필수 사항)] HTTP Status Code: \(httpResponse.statusCode)")
                ...
                if let extractedContent = self?.extractAgreementContent(from: htmlString) {
                    ...
                    self?.locationBasedServiceConsentWebView.loadHTMLString(formattedHTML, baseURL: nil)
                } else {
    ...
    private func extractAgreementContent(from htmlString: String) -> String? {
        ...
        for pattern in patterns {
            do {
                ...
                print("[개인위치기반서비스 이용동의(필수 사항)]정규식 오류: \(error.localizedDescription)")
            }
        }
        
        if let divContent = extractDivContent(from: htmlString, divId: "ag5") {
            return divContent
        }
